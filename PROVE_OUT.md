...
| **3** | **full fan-out** — long-tail manifest, tested concurrency pool, run by tier | continuous aggregation; A/B clusters fed back to the binder; `--rerun-failures` on each binder commit | ✅ done (36/36 E) |
| **4** | **emit lane: production-toolchain source codegen** — the binder's second backend (`nb::emit_bindings`, nanobind `nb_reflect_emit.h` + `nb_reflect_spell.h`) renders the SAME binding decisions as ordinary nanobind source; the P2996 toolchain is needed only at GENERATION time, the module is compiled by Apple Clang + system libc++ (the deployability story). Every migrated run validates **three-way**: native oracle, constexpr module, emit module must agree, plus a Gate 6b module-vs-module API-surface diff (`surface_diff.py`). New taxonomy: `B.emit_gen_compile/_gen_run` (stage 1, reflection toolchain), `B.emit_compile/_extra_compile/_link` (stage 2, production compiler), `D.surface`. Rollout waves: E0 fixtures → E1 glm+json (driver-direct shakeout) → E2+ remaining 33 in risk-ordered waves via Opus-worker ⇄ Fable-reviewer iteration (`AGENT_PROMPT_EMIT.md`); a run counts only when BOTH lanes are green. | ✅ **done (36/36 three-way E, zero surface ignores)** -- every run's emit module is generated by the P2996 toolchain and compiled/linked by Apple Clang + system libc++, passes the SAME differential pytest suite against the SAME oracle as the constexpr module, and is surface-identical. New findings driven by this phase, ALL fixed locally and FILED upstream: BINDER-0029 (chrono caster map, caught by Gate 6b), the wchar_t/char-family and __int128 spelling gaps, **TC-0018** (2^15-element packs miscompile -- root-caused to SubstNonTypeTemplateParmPackExpr's 15-bit argument count, reproducible in plain C++; #321/PR #323), **TC-0019** (symbol_of's "^=" table row reads "^"; #319/PR #320, new tracker category F), **TC-0020** (parameter identifier_of was instantiation-state-dependent; #322/PR #324 -- the fix removed eigen's setConstant surface ignore, the campaign's only Gate 6b triage). Hardening: the emit-mode SPELLING-PROBE TU (`nb::write_spelling_probe`, the cast-based round-trip oracle for nb_reflect_spell.h) is wired into the unit build. Post-fix full re-sweep: 36/36 E. |
...
(#302/PR #305, #303/PR #306, #304/PR #307); **TC-0013..0017 are upstreamed**
(the wave-2 batch: #309/PR #310, #311/PR #315, #312/PR #316 stacked on #301,
#313/PR #317 stacked on #306, #314/PR #318); **TC-0018..0020 are upstreamed**
(the Phase-4 batch: #321/PR #323 category E, #319/PR #320 category F — new:
std::meta library data, #322/PR #324 category B; each PR-state-validated on
base 837da39 + fix with clang/test/Reflection at base parity, evidence in
`repros/TC-00NN/UPSTREAM.md`) — all tracked on the campaign
