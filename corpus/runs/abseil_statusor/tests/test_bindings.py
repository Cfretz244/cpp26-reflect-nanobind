...
directly: ok(), status() -> bound absl::Status, IgnoreError(). value() does NOT bind:
it is a using-redeclaration from the PRIVATE internal_statusor::OperatorBase base, and
the entity-proxy feature that used to bind it (BINDER-0009, the clang fork's
-fentity-proxy-reflection extension) was removed after P3687R1 deferred
shadow-declaration reflection past C++26. Values are read through the get_* fixture
channel instead. Construction still goes through the sotest factories (StatusOr's
converting ctors are templates, skipped by design). oracle_native.cpp drives the
identical surface.
"""
...
def test_ok_int_differential():
    ...
    assert not hasattr(oi, "value")             # private-base re-export: not bound
    assert m.get_int(oi) == E["oi_value"]       # fixture channel
...
def test_value_on_error_raises():
    # value() on an error StatusOr throws absl::BadStatusOrAccess -> Python
    # exception through the fixture channel (the direct value() binding is gone
    # with the entity-proxy feature).
