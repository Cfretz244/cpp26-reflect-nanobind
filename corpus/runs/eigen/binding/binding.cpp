...
consteval void collect_members_named(sm::info M, std::string_view name,
    ...
    nb::detail::collect_public_base_subtree(M, owners);
    for (auto o : owners)
        for (auto m : sm::members_of(o, sm::access_context::unchecked()))
            if (!nb::detail::is_using_proxy(m) && sm::has_identifier(m) &&
...
consteval void collect_subscript_operators(sm::info M,
    ...
    for (auto o : owners)
        for (auto m : sm::members_of(o, sm::access_context::unchecked()))
            if (!nb::detail::is_using_proxy(m) && sm::is_function(m) &&
...
NB_MODULE(eigen_ext, m) {
    nb::reflect_<^^Eigen::Matrix<double, 3, 1>, ^^Eigen::Matrix<double, 3, 3>,
                 eigen_excluded_marker(), ^^eigentest>(m);
}
